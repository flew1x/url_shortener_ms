...
func NewURLService(logger *slog.Logger, urlRepository repository.IURLRepository, cache cache.IUrlCache, config config.IConfig) IURLService {
...
func (l *urlService) generateShortUrl(length int) url.URL {
	...
	url := url.URL{
		Scheme: l.config.GetServerConfig().GetScheme(),
		Host:   l.config.GetServerConfig().GetBindIP(),
		Path:   string(b),
	}
...
func (s *urlService) Create(ctx context.Context, originURL string) (shortURL string, err error) {
	...
	shortGeneratedURL := s.generateShortUrl(s.config.GetUrlConfig().LengthShortURL())
